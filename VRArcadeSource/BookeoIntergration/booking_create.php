...
$sql = "INSERT INTO `Bookings` (`booking_id`,`booking_start_time`,`booking_end_time`,`customer_id`,`booking_num_total`,`customer_name`,`customer_email`,`customer_phone`,`booking_creation_time`,`booking_prod_name`,`booking_prod_id`,`total_paid`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
$q = $pdo->prepare($sql);
$q->execute(array($booking_id,$booking_start_time,$booking_end_time,$customer_id,$booking_num_total,$customer_name,$customer_email,$customer_phone,$booking_creation_time,$booking_prod_name,$booking_prod_id,$total_paid));
Database::disconnect();
